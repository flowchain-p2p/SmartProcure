...
const submitRequisition = async (req, res) => {
  try {
    ...
    if (requisition.createdBy.toString() !== req.user.id && !['admin'].includes(req.user.role)) {
      return res.status(403).json({
        success: false,
...
const approveRequisition = async (req, res) => {
  try {
    ...
    // Check if requisition is in Submitted or Under Review status
    if (!['Submitted', 'Under Review'].includes(requisition.status)) {
      return res.status(400).json({
        success: false,
        error: 'Only submitted or under review requisitions can be approved/rejected'
      });
    }
...
const deleteRequisitionItem = async (req, res) => {
  try {
    ...
    // The post-remove hook will update the total amount in the requisition
